...
ralph-jev runs your coding agent (Claude Code, Codex, Gemini CLI, Kiro and others) in a loop, with a fresh context on every iteration. State lives on disk and in git: events, tasks, memories and the objective. It is inspired by the Ralph loop technique (`while :; do cat PROMPT.md | agent; done`).

The difference is who decides that the work is finished. The agent can only *claim* completion. An external judge backed by [TypeSafe Jev](https://typesafe.ai) decide whether the claim holds, and the loop keeps going with the reason whenever it doesn't.
...
| [Completion gate](concepts/completion-gate.md) | How completion is judged |
