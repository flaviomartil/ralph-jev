...
The judge, the verify command and the loop's judge call all run in their own process groups. On timeout the whole group is killed, and output kept in memory is capped, so a runaway command can't hang or flood the loop.
