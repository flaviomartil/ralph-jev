...
1. `ralph-jev run` resolves the objective: `-p` text, `-P` file, the config `prompt`/`prompt_file`, or `PROMPT.md`.
...
When the agent emits `LOOP_COMPLETE`, the loop first runs its built-in checks (required events seen, human guidance acknowledged, no open tasks). Then it calls the external judge configured in `event_loop.completion_judge`, `ralph-jev-judge` by default. See [Completion gate](../concepts/completion-gate.md).
