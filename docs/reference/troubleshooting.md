...
`No supported AI backend found in PATH.` Install a builder CLI (Claude Code, Codex, Gemini CLI, Kiro, ...) and make sure it is on `PATH`, then run `ralph-jev doctor`. To use a specific one, set `cli.backend` in `ralph.yml`.
...
`cli.backend: custom` needs `cli.command`. Set it in `ralph.yml`, or generate a starting config with `ralph-jev init --backend custom`.
...
The Telegram robot (`RObot`) section has an invalid or missing field. The error names the field and gives a hint; fix it or disable the robot. `ralph-jev bot --help` has the setup commands.

## Config file exists

`ralph-jev init` won't overwrite an existing `ralph.yml`. Pass `--force` to replace it.

## Unknown preset

Monolithic presets were replaced by split config. Generate core config with `ralph-jev init --backend <backend>`, pick a hat collection from `ralph-jev init --list-presets`, and run with `-H builtin:<collection>`.

## The judge rejects work that looks done

- Read the reason in the loop output or `ralph-jev events`. It names the gap and the next step.
...
- Check that `event_loop.completion_judge.command` is set in the config that is actually loaded (`ralph-jev preflight` checks the loaded config).
