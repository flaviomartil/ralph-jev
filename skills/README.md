...
- `ralph-docs` for introspecting and improving Ralph itself via the
  ...
  questions and scoping code changes to the ralph-jev repo
...
```text
/plugin marketplace add flaviomartil/ralph-jev
```

Then install the `ralph-jev` plugin from the marketplace browser.
...
```bash
npx skills add flaviomartil/ralph-jev --list
```

Install all skills for Claude Code:

```bash
npx skills add flaviomartil/ralph-jev \
  --skill ralph-hats \
  --skill ralph-loop \
  --skill ralph-docs \
  -a claude-code \
